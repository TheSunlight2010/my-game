...
const additionalCharacters = {
    ...
    null: {
        ...
        health: 100,
        variable: true,
        // bio intentionally left blank for this entity
        bio: "I don't know what this is.",
        // abilities are assigned at battle start by runtime; placeholder to satisfy tools
        abilities: []
    },
...
function classifyCharacterByHealing(char, keyName = '') {
    ...
    const varyKeys = ['seven', 'reflection', 'wachi', 'eteleD', 'fyre', 'judgement', 'null']; // add entries as needed
...
/*
  Helper: compute healing multiplier for a healer against a defender.
  Rule: If the acting character is classified as SELF-SUPPORT and the opponent's
  main classification includes BRAWN, healing is 2.5x effective.
*/
function computeHealingMultiplier(actorObj, opponentObj, actorKeyName) {
    try {
        // Prefer using existing classifier which accepts (char, keyName)
        ...
        // Extract main class before the semicolon (e.g. "SELF-SUPPORT; REGEN")
        ...
        if (actorMain === 'SELF-SUPPORT' && opponentMain === 'BRAWN') {
            return 2.5;
        }
    } catch (e) {
        // fallback to no multiplier on error
    }
    return 1;
}

 // ----- BLEEDING SYSTEM HELPERS -----
// gameState.bleeds holds bleed objects for sides: { player: {...} , enemy: {...} }
// Each bleed: { damagePerTick: number, active: bool, source: string }
// Internal interval references stored on window.__bleedIntervals to allow clear
window.__bleedIntervals = window.__bleedIntervals || {};
// KARMA helpers: karmas stored similar to bleeds; interval refs on window.__karmaIntervals
window.__karmaIntervals = window.__karmaIntervals || {};

 // Start a bleed on side: 'player' or 'enemy'; totalAmount is the total bleed to apply over time (1 HP/sec)
 // Bleed will stop early if the bleeding target's current HP drops below the opposing combatant's first ability damage value.
function startBleed(side, totalAmount, source = '') {
    if (!['player','enemy'].includes(side)) return;
    try {
        // Ensure totalAmount is at least 1
        const total = Math.max(1, Math.floor(totalAmount || 1));
        // replace existing bleed with new values (accumulated resets)
        gameState.bleeds[side] = { perTick: 1, totalRemaining: total, accumulated: 0, active: true, source: source || '' };

        // clear any existing interval for this side
        if (window.__bleedIntervals[side]) {
            clearInterval(window.__bleedIntervals[side]);
            window.__bleedIntervals[side] = null;
        }

        // Helper to get the opponent's first ability damage (fallback to 0)
        ...
        // create a tick interval that removes exactly 1 HP per second until the total is exhausted
        window.__bleedIntervals[side] = setInterval(() => {
            try {
                ...
                // Determine the bleeding target current HP reference and the opponent's first-ability threshold
                const targetIsPlayer = (side === 'player');
                const targetHP = targetIsPlayer ? gameState.playerHealth : gameState.enemyHealth;
                const threshold = getOpponentsFirstAbilityDamage(side); // if target goes under this, stop bleed

                // If the bleeding target's HP is below the opposing user's first ability damage, stop bleeding early
                ...
                if (side === 'player') {
                    ...
                    gameState.enemyHealth = Math.max(0, gameState.enemyHealth - tick);
                }

                // update accumulated and remaining
                ...
                // If total exhausted or HP reaches zero, clear bleed and interval
                if (bleed.totalRemaining <= 0) {
                    // mark inactive and clear interval
                    bleed.active = false;
                    clearInterval(window.__bleedIntervals[side]);
                    window.__bleedIntervals[side] = null;
                    // ensure UI reflects bleed cleared
                    try { updateBattleUI(); } catch (e) {}
                    return;
                }

                // If HP reaches zero, end battle accordingly
...
// ----- KARMA SYSTEM HELPERS -----
// KARMA: applied only by Judgement to non-pacifist opponents (nullifies immediate damage and applies it over time).
// Structure: gameState.karmas.side = { total: number, accumulated: number, active: bool, source: string }
// Timing: 50% of total applied at 2.2s, remaining 50% applied evenly over ~14.17s (approx 14 ticks of 1s).
function startKarma(side, totalAmount, source = '') {
    if (!['player','enemy'].includes(side)) return;
    try {
        const total = Math.max(1, Math.floor(totalAmount || 1));
        // replace existing karma on that side
        ...
        // clear any existing karma interval for this side
        ...
        // New behavior:
        // Split total into two phases:
        //  - Phase A: "initial" half executed as small ticks distributed evenly across 2200ms
        //  - Phase B: remaining half distributed over ~14.17s as before
        const half = Math.floor(total * 0.5);
        const rest = total - half;

        // PHASE A: distribute `half` over 2200ms in N ticks for smooth overtime damage.
        // Choose number of ticks based on half (but clamp to reasonable bounds)
        const minTicks = 4;
        const maxTicks = 22; // finer granularity if half large
        const ticksA = Math.max(minTicks, Math.min(maxTicks, half || minTicks));
        const intervalAms = Math.max(60, Math.floor(2200 / ticksA)); // at least 60ms per tick
        // Determine per-tick base and remainder distribution
        const perTickA = Math.floor(half / ticksA);
        const remA = half - perTickA * ticksA;
        let ticksADone = 0;

        // apply small ticks over 2200ms window
        window.__karmaIntervals[side].initialInterval = setInterval(() => {
            try {
                let apply = perTickA;
                if (ticksADone < remA) apply += 1; // distribute remainder in earliest ticks
                if (apply > 0) {
                    if (side === 'player') {
                        gameState.playerHealth = Math.max(0, gameState.playerHealth - apply);
                    } else {
                        gameState.enemyHealth = Math.max(0, gameState.enemyHealth - apply);
                    }
                ...
                if (ticksADone >= ticksA) {
                    // finished phase A; clear interval and start phase B after ensuring timing completes ~2200ms
                    clearInterval(window.__karmaIntervals[side].initialInterval);
                    window.__karmaIntervals[side].initialInterval = null;

                    // Start PHASE B (remaining) using the same timing window as phase A (2200ms),
                    // i.e. use the same number of ticks and per-tick cadence for consistency.
                    if (rest > 0) {
                        const ticksB = ticksA;
                        ...
                        window.__karmaIntervals[side].remaining = setInterval(() => {
                            try {
                                ...
                                if (side === 'player') {
                                    ...
                                    gameState.enemyHealth = Math.max(0, gameState.enemyHealth - applyB);
                                }
                        ...
                        // if no rest remaining, mark inactive
                        gameState.karmas[side].active = false;
                        updateBattleUI();
                    }
                }
                ...
                try { clearInterval(window.__karmaIntervals[side].initialInterval); } catch (e) {}
                window.__karmaIntervals[side].initialInterval = null;
                // fallback to scheduling remaining immediately if any
                if (rest > 0) {
                    // Begin remaining distribution using the same tick count and cadence as phase A
                    const ticksB = ticksA;
                    const intervalBms = intervalAms;
                    const perTickB = Math.floor(rest / ticksB);
                    const remB = rest - perTickB * ticksB;
        ...
        // Safety: ensure update immediately so UI shows karma active from the start
        updateBattleUI();
    } catch (e) {
...
function getSpecialBattleTheme(playerName, enemyName) {
    ...
    const matchups = [
        ...
        // Drew vs Daphne (special theme) — use Fallen Angel
    ...
    // Add C_11 vs Tao special case (plays Back Again)
...
let gameState = {
    ...
    // Bleeding status: { player: { damagePerTick: number, active: bool, source: string }, enemy: { ... } }
    bleeds: { player: null, enemy: null }
};

// Load saved data
/*
  Try importing and running dean.js with retries to handle intermittent module load failures.
  - call tryImportDean(onReload) to attempt import; it will retry every 2000ms up to maxAttempts (default 8)
  - once import succeeds, it executes the module default with the provided onReload flag
  - failures are logged; deanPending is left in storage so retries persist across reloads
*/
window.tryImportDean = function tryImportDean(onReload, maxAttempts = 8, intervalMs = 2000) {
    // avoid creating multiple parallel retry loops
...
/*
  assignNullAbilities
  - When called with the 'null' character object (additionalCharacters.null), selects up to 4 abilities
    sampled from other characters' abilities pools (excluding null itself). Ensures at least two of the
    assigned abilities are damaging (damage >= 0). Assigned abilities are shallow-copies to avoid
    mutating originals and will be used at runtime (battle start).
*/
function assignNullAbilities(nullCharObj) {
    try {
        if (!nullCharObj) return;
        // If null already has abilities and they're non-empty, don't reassign unless empty.
        if (Array.isArray(nullCharObj.abilities) && nullCharObj.abilities.length >= 4) return;

        // Build pool of candidate abilities from all characters except 'null'
        ...
        Object.keys(combined).forEach(k => {
            ...
            c.abilities.forEach(a => {
                // shallow clone ability, keep source reference for provenance if useful
                pool.push(Object.assign({}, a, { from: k }));
            });
        });

        if (pool.length === 0) {
            // fallback: simple placeholder abilities so null is functional
        ...
        // Attempt to pick up to 4 abilities, guaranteeing at least 2 damaging ones (damage >= 0)
        const chosen = [];
        let damageCount = 0;
        let idx = 0;

        // First pass: prefer picking damaging abilities until we have 2
        ...
        // Second pass: fill remaining slots with any abilities (damage or heal) but avoid duplicates by name
        idx = 0;
        ...
        // If still don't have 2 damage abilities (rare), replace last entries with damaging ones from pool
        if (damageCount < 2) {
        ...
        // Final trim/pad to exactly 4 abilities
        // If fewer than 4, append clones of the first pool entries (deduped by name)
        let padIdx = 0;
        while (chosen.length < 4 && padIdx < pool.length) {
            if (!chosen.some(c => c.name === pool[padIdx].name)) chosen.push(Object.assign({}, pool[padIdx]));
            padIdx++;
        }
        // If still underfilled (pathological), add simple attacks
        while (chosen.length < 4) {
            chosen.push({ name: "Null Scribble", damage: 10, description: "A generic strike" });
        }
        // If more than 4 (shouldn't be), truncate
...
function loadGameData() {
    try {
        ...
        if (saved) {
            ...
            // Preserve dean-related flags from save
        ...
        // If a dean pending flag exists in the loaded save, attempt to start the dean sequence on page load.
        // Important: only clear the persisted deanPending flag after the module successfully loads and runs,
        // so a failed import won't make the sequence permanently disappear.
        try {
            const saved = JSON.parse(localStorage.getItem('unbrokenSave') || '{}');
            if (saved && saved.deanPending) {
                // import and run dean.js with the reload variant (true) so it uses the alternate text
                import('./dean.js').then(mod => {
                    try {
                        if (typeof mod.default === 'function') {
                    // run dean with the reload variant
                    mod.default(true);
                }
                    } catch (e) {
                        console.error('dean.js executed but failed', e);
                    ...
                    // If loading fails, keep the flag so the loader can retry on the next reload.
...
function saveGameData() {
    try {
        const data = {
            ...
            // Preserve dean-related ephemeral state so dean sequence can persist across reloads
...
function init() {
    ...
    // Extras button: opens the Extras screen (for miscellaneous content)
    const extrasButton = document.getElementById('extras-button');
    if (extrasButton) {
        extrasButton.addEventListener('click', () => {
            showScreen('extras');
            // Initialize audio play handlers for music list
            try {
                const list = document.getElementById('music-list');
                if (!list) return;
                // ensure previous handlers are not duplicated
    ...
    // Business button: opens Business' Business modal (dynamic module)
    ...
    // Extras back button (if present) returns to title
...
function startBattle(playerCharacterKey) {
    ...
    // If the player chosen is the special 'null' character, ensure it receives a random set of abilities
    if (playerCharacterKey === 'null' && window.additionalCharacters && window.additionalCharacters.null) {
        ...
        // Ensure the gameState.player reference points to the updated object used for battles
    ...
    // If the randomly selected enemy is the special 'null' character, assign abilities now so it has borrowed moves
    if (randomEnemyKey === 'null' && window.additionalCharacters && window.additionalCharacters.null) {
        try { assignNullAbilities(window.additionalCharacters.null); } catch (e) { console.error(e); }
        // refresh the enemy reference to the updated object
        gameState.enemy = additionalCharacters['null'];
    }

    // --- New: 1-in-5 special Luna infection event before battle starts ---
    try {
        // Conditions that prevent the special event:
        //  - If player or enemy is Awareness
        //  - If the player has only the default trio unlocked (cupiditas, kite, subject192) — i.e. no other unlocks
        //  - If the chosen enemy is Fossil or Subject 192 (they cannot be targeted by Luna)
        ...
        // check the name of the selected enemy as well (gameState.enemy may not yet be fully mirrored into characters table)
        ...
        // 0% chance if only defaults, awareness is involved, or the enemy is Fossil/Subject 192
        let triggerChance = 0;
        if (!onlyDefaults && !playerIsAwareness && !enemyIsAwareness && !enemyIsFossilOrSubject192) {
            triggerChance = 0.20; // 1 in 5
        }

        if (Math.random() < triggerChance) {
            // Ask player with a simple confirmation dialog before the battle begins
            // (spec requested a warning popup and then "OK" to begin)
            const ok = window.confirm("Luna: Yo, freak! I wanna fight! A challenge! If you win, you get 150 Credits!");
            // If player dismisses (Cancel), we still start the battle but without giving the reward promise; infection only applies on OK
            if (ok) {
                gameState.lunaInfected = true;
                // mark the infected side: enemy is infected for the battle
    ...
    // If Luna infected this battle and the enemy is the target, boost enemy max HP by 1.5x now (and clamp current HP to new max).
    // Also adjust enemy abilities' damage/heal scaling in-place so runtime ability resolution will reflect the infection.
    if (gameState.lunaInfected && gameState.lunaInfectedTarget === 'enemy' && gameState.enemy) {
        try {
            // multiply max health by 1.5 and set current health proportionally
            ...
            // set current HP to full infected max at start
            gameState.enemyHealth = infectedMax;

            // apply per-ability multipliers: enemy deals 1.3x damage, heals 0.9x
    ...
    // Special case: Prime should never display her wounded sprite when facing Blitz.
    // Set a short-lived flags so updateBattleUI can honor this exception without toggling house-arrest behavior.
    try {
        ...
        // primeKeepNormal ensures Prime never shows wounded sprite in this matchup
        gameState.primeKeepNormal = !!isPrimeBlitz;

        // primeBlitzHouseLike indicates we should apply house-arrest-like sprite handling for this matchup
        gameState.primeBlitzHouseLike = !!isPrimeBlitz;

        // Ensure Prime's sprite stays the normal variant when this matchup occurs
        if (isPrimeBlitz) {
        ...
        // When Prime faces Blitz, give Blitz the Fallen Angel themed sprites for this matchup.
        // This will swap Blitz's visible portrait to the alternate "fallen angel" artwork.
        if (isPrimeBlitz) {
...
function playBattleMusic(level, isHouseArrestBattle = false) {
    ...
    // If Luna challenged the player pre-battle (lunaInfected flag), force the Luna mix and prevent overrides.
    try {
        ...
        // fall through to normal selection if check errors
    }
...
function updateBattleUI() {
    ...
    // Compute desired sprite selection first, allowing KARMA to override below.
    let desiredPlayerSprite = gameState.player.sprite;
    let desiredEnemySprite = gameState.enemy.sprite;

    // If a matchup forces wounded sprites, always prefer wounded portraits regardless of health
    if (gameState.forceWoundedSprites) {
        ...
        // Prime vs Blitz: Prime forced normal; others respect wounded thresholds
        ...
        // Prime-exception general fallback: Prime stays normal, others use wounded thresholds
        desiredPlayerSprite = (gameState.player && gameState.player.name === 'Prime') ?
            gameState.player.sprite :
            (gameState.playerHealth > 50 ? gameState.player.sprite : (gameState.player.woundedSprite || gameState.player.sprite));
        ...
        // Use House Arrest sprites (respect wounded state by health)
        desiredPlayerSprite = gameState.playerHealth > 50 ? gameState.player.sprite : (gameState.player.woundedSprite || gameState.player.sprite);
        desiredEnemySprite = gameState.enemyHealth > 50 ? gameState.enemy.sprite : (gameState.enemy.woundedSprite || gameState.enemy.sprite);
    } else {
        // Normal behavior: use wounded sprite when below threshold
        desiredPlayerSprite = gameState.playerHealth > 50 ? gameState.player.sprite : (gameState.player.woundedSprite || gameState.player.sprite);
        desiredEnemySprite = gameState.enemyHealth > 50 ? gameState.enemy.sprite : (gameState.enemy.woundedSprite || gameState.enemy.sprite);
    }

    // KARMA override: if KARMA is active on a side, force the wounded sprite when available
    try {
        ...
        // ignore errors and proceed with previously chosen sprites
        console.error('KARMA sprite override failed', e);
    }

    // Apply the computed sprite choices to the DOM
    playerSprite.src = desiredPlayerSprite;
    enemySprite.src = desiredEnemySprite;

    // If Luna infected the battle targeting the enemy, add a purple sparkle/glow class to the enemy sprite for visual emphasis.
    try {
    ...
    // Toggle bleeding visuals: if either side has an active bleed, add .bleeding to its health-bar container.
    try {
        ...
        // ignore DOM errors but don't break the rest of the UI update
    ...
    gameState.player.abilities.forEach((ability, index) => {
        ...
        // Toggle healing visual class when ability is a heal (damage < 0)
        try {
...
function useAbility(abilityIndex) {
    ...
    if (actualDamage > 0) {
        ...
        if (gameState.player && gameState.player.name === 'Thirteen' && ability.name === 'Luck Drain') {
            ...
            // Only Judgement's specific "Bone Barrage" ability applies KARMA; other Judgement moves act normally.
            try {
                ...
                if (actorName === 'Judgement' && abilityNameSafe === 'Bone Barrage' && !isPacifistTarget) {
                    // Nullify immediate damage and convert into KARMA overtime equal to intended damage
                    const intended = actualDamage;
                    addToBattleLog(`${gameState.player.name} uses ${ability.name} and inflicts KARMA (${intended}) on ${gameState.enemy.name}!`);
                    startKarma('enemy', intended, ability.name || '');
                } else {
                    // Normal immediate damage behavior
                    ...
                    // Determine whether this ability should apply Bleeding:
                    // - ability damage is weaker than the character's first ability
                    // - exclude Awareness, Martial Artist, and Seven as actors
                    try {
                        ...
                        if (!actorExcluded && isWeakerThanFirst) {
                            // Goal: total bleed equals half the ability's normal damage (rounded down, at least 1),
                            // applied as 1 HP per second until the total is reached.
                ...
                // fallback: ensure damage still applied
                gameState.enemyHealth = Math.max(0, gameState.enemyHealth - actualDamage);
                addToBattleLog(`${gameState.player.name} uses ${ability.name} for ${actualDamage} damage!`);
            }
        }
    } else {
        // Healing path (player used a heal)
        const playerBattleMaxClamp = (gameState.playerBattleMax || gameState.player.health);

        // Determine base heal amount (positive number)
        let healAmount = -actualDamage;

        // Compute multiplier: SELF-SUPPORT healers heal 2.5x when fighting a BRAWN opponent
        const actorDef = gameState.player;
        const opponentDef = gameState.enemy;
        // Try to identify the player's key name if possible (from characters/additionalCharacters)
        ...
        const multiplier = computeHealingMultiplier(actorDef, opponentDef, actorKey);

        const baseHeal = Math.round((-ability.damage));
        healAmount = Math.round(healAmount * multiplier);
        // Cap self-support healing: allow cap of 43 unless the ability's base heal already exceeds 43
        const healCap = Math.max(43, baseHeal);
        healAmount = Math.min(healAmount, healCap);
        ...
        // Clearing bleed on player when they heal
        try {
            clearBleed('player');
        } catch (e) {}

        // Subject 192 special: Cannibalism reduces max HP when used (still heals normally)
        try {
            if (gameState.player && gameState.player.name === 'Subject 192' && (ability.name || '').toLowerCase() === 'cannibalism') {
                // base reduction 5; if multiplier > 1 (SELF-SUPPORT vs BRAWN case) double to 10
                const reductionBase = 5;
                const reduction = Math.round(reductionBase * (multiplier > 1 ? 2 : 1));
                // reduce battle max and current max reference safely, ensuring at least 10 max remains
                ...
                // adjust both the character object's health max and current HP if necessary
                if (gameState.player) gameState.player.health = newMax;
                // Ensure current HP does not exceed new max (healing still applied above)
...
function enemyTurn() {
    ...
    // Build available indexes excluding abilities on cooldown. Additionally, if the acting enemy is P,
    // explicitly forbid selecting the "Battery" ability so the enemy cannot use that overpowered heal.
    let availableIndexes = enemyAbilities.map((a, i) => i)
        .filter(i => !(enemyCDs[i] > 0));

    let chosenIndex;
    // If there are abilities not on cooldown (and not forbidden), pick one at random from them.
    if (availableIndexes.length > 0) {
        chosenIndex = availableIndexes[Math.floor(Math.random() * availableIndexes.length)];
    } else {
        // All abilities (after filtering) are unavailable; fall back to picking the least-cooldown ability
        // but still avoid selecting P's Battery if possible.
        let minCd = Infinity;
        let minIdx = 0;
        for (let i = 0; i < enemyCDs.length; i++) {
            const cd = enemyCDs[i] || 0;
            // If enemy is P, skip Battery when choosing fallback unless it's the only option.
        ...
        // If minIdx still points to an ineligible Battery (or nothing found), allow the engine to pick any minimal-cd index.
        if (minCd === Infinity) {
            // fallback to original behavior: pick the ability with the smallest remaining cooldown (including Battery if unavoidable)
            minCd = Infinity;
            for (let i = 0; i < enemyCDs.length; i++) {
                const cd = enemyCDs[i] || 0;
                if (cd < minCd) {
    ...
    if (actualDamage > 0) {
        ...
        // Determine whether this enemy ability should apply Bleeding to the player:
        // - ability damage is weaker than the enemy's first ability
        // - exclude Awareness, Martial Artist, and Seven as actors
        try {
        ...
        // Healing path (enemy used a heal)
        ...
        healAmount = Math.round(healAmount * multiplier);
        // Cap self-support healing passive so a heal never exceeds 43 HP
        healAmount = Math.min(healAmount, 43);
        ...
        // Clear bleed on enemy when they heal (mirror player heal behavior)
        try { clearBleed('enemy'); } catch (e) {}

        // Subject 192 special when acting as enemy: Cannibalism reduces max HP when used (still heals normally)
        try {
            if (gameState.enemy && gameState.enemy.name === 'Subject 192' && (randomAbility.name || '').toLowerCase() === 'cannibalism') {
                ...
                const newMax = Math.max(10, prevMax - reduction);
                gameState.enemy.health = newMax;
                // clamp current HP to new max (healing already applied above)
...
function endBattle(playerWon) {
    ...
    // Reset House Arrest flag and any prime-vs-blitz house-like sprite flag
    gameState.isHouseArrestBattle = false;
    gameState.primeBlitzHouseLike = false;
    
    if (playerWon) {
        // compute reward (base + any active bonuses)
        let earnedCredits = 50; // base reward

        // If this battle was the Luna-infected special (player accepted), award the promised 150 credits as a bonus.
        if (gameState.lunaInfected && gameState.lunaInfectedTarget === 'enemy') {
            earnedCredits = 150;
            // award the promised 150 credits total
            gameState.credits += earnedCredits;
            addToBattleLog(`Special reward: +150 Credits for defeating the Luna-infected enemy!`);
            // clear the luna infection flag so bonus can't be claimed twice
            gameState.lunaInfected = false;
            gameState.lunaInfectedTarget = null;
        } else {
            // no special event: award base only
            gameState.credits += earnedCredits;
        }
        ...
        // Display the correct earned amount in the victory log (use earnedCredits for popup)
        addToBattleLog(`Victory! +${earnedCredits} Credits earned!`);
        
        // Show victory screen after a brief delay and pass the earned amount for accurate display
...
function showVictoryScreen(earnedCredits = 50) {
...
function checkAllUnlockedAndTriggerEasterEgg() {
    try {
        ...
        if (allUnlocked) {
            ...
            // Also hide the "ROLL" business button and the update notice so the title screen is simplified
            const idsToHide = ['start-game', 'characters-button', 'shop-button', 'credits-display-title', 'business-button', 'update-notice'];
...
window.additionalCharacters = additionalCharacters;
 ...
 init();
 // Expose gameState globally so external modules (like business.js) can access and modify it
 try { window.gameState = gameState; } catch (e) { /* ignore */ }

 // Ensure the dean reload message runs reliably on page load if the persisted flag remains.
 // This duplicates the earlier attempt but runs after full load and is defensive against race conditions.
 (function tryRunDeanOnLoad(){
     // Avoid running multiple times
     if (window.__deanRunAttempted) return;
     window.__deanRunAttempted = true;

     // Use the load event so DOM is settled and any overlays can be created safely.
     window.addEventListener('load', () => {
         try {
             const saved = JSON.parse(localStorage.getItem('unbrokenSave') || '{}');
             if (saved && saved.deanPending) {
                 // Try to import and run dean.js with onReload=true (the "Where are you going?" line)
                 // Use the resilient importer that retries on failure (deferred attempt on load)
                tryImportDean(true);
             }
         } catch (e) {
